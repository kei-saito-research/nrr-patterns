...
and task conditions. This paper compares five delayed-commitment patterns on a
fixed two-run matched paired comparison spine with live provider APIs and then
reports the released outputs through a provider-separated stability read, a
separated temperature-robustness member, and a selected Stage B
boundary-honesty carry-forward. The main comparison map is carried by the
fixed matched-run spine, while sign-flip and non-effective regions are carried
by separately labeled selected Stage B artifacts rather than folded into the
headline comparison itself. On the fixed matched-run spine, Hierarchical
Resolution, Delta Update, Lazy Expansion, Conditional Refinement, and Branch
Preservation are all positive relative to their own paired baselines at the
provider-balanced aggregate level, while Branch remains only weakly positive on
that aggregate read. The provider-separated view is less uniform: Branch
already contains negative Anthropic members on the fixed spine before failing
Stage2, and Conditional weakens under Stage2 while retaining a positive
provider-balanced aggregate despite an Anthropic-side reversal. The separated
temperature-robustness member then shows that Branch is the clearest fragile
case, but the failure shape is tail-risk-dominated rather than provider-wide:
the Gemini-side negative mean is driven mainly by one catastrophic row, while
the remaining Gemini rows are near-neutral in aggregate. A compact compression
comparison over a retained three-pattern subset shows that the observed gains
for that tested subset are not reducible to generic prompt shortening:
principle-only remains clearly positive within each tested family-relative
contrast, whereas compression-only weakens or fails for key cases. Selected
Stage B boundary checks then show that a single aggregate comparison view can
hide provider-sensitive reversals and non-effective cells. On the retained key
metrics, Conditional has the clearest positive provider-mean carry-forward, but
that carry-forward remains sparse rather than uniformly active, while Lazy is
the clearest degraded case and Hierarchical, Delta, and Branch show
provider-sensitive weakening or reversal cells. A separate selected
non-effective slice then shows all-provider-zero cells for Conditional, Delta,
and Branch on internal-coherence and trace-completeness diagnostics. The
resulting contribution is a condition-bounded comparison whose comparison
headline is kept honest by explicit provider-separated stability, tail-risk
disclosure, robustness, and separately labeled sign-flip and non-effective
reporting rather than by a same-scale cross-family ranking or universal
deployment rule.
...
five tested delayed-commitment patterns on one auditable fixed matched-run
comparison spine while keeping the comparison evidence separate from the
selected Stage B honesty carry-forward used for sign-flip and non-effective
reporting. In this paper, the headline comparison comes from the fixed matched
\(t=0.0\) experiment spine and the released outputs traced from it, while the
selected Stage B honesty layer remains a separate carry-forward caveat rather
than part of the headline spine.

This NRR-Patterns manuscript is the comparison-led pattern paper in the
current NRR spine. Fixed public lines in that spine establish first principles
and text-to-state mapping~\cite{saito2025nrr,saito2026phi}. Additional IME,
Transfer, and Coupled lines remain series-path references rather than fixed
evidentiary supports for the claims made here~\cite{saito2026ime,saito2026transfer,saito2026coupled}. The role here is narrower and more practical:
compare five delayed-commitment patterns on a fixed matched-run paired spine,
then report the resulting evidence through provider-separated stability reads,
a separated temperature-robustness member, and a separately labeled selected
Stage B honesty layer. In the downstream series read, this paper is the
upstream comparison-and-boundary-honesty layer for later calibration and
closure work, not a universal endpoint.
...
The primary experimental spine uses the exact-replication five-pattern
comparison currently carried by the released NRR-Patterns line. Here, exact
replication continues to mean the fixed auditable matched-run outputs named in
the shipped artifacts, not a rerun-capable environment. The shipped
...
These two \(t=0.0\) runs define the headline exact-replication spine for the
current NRR-Patterns comparison read. The current NRR-Patterns source-of-truth
name freeze for these comparisons is recorded in
\path{repro/nrr_patterns_comparison_claims_manifest_v7_2026-04-12.csv}, and
the paper-local fairness/aggregation contract for separating headline
replication from robustness is frozen in
...
The compression comparison remains in the main manuscript as a compact interpretive control. Its role is to answer the reviewer question: are the observed gains just generic prompt compression? The canonical NRR-Patterns name for this control is also frozen in \path{repro/nrr_patterns_comparison_claims_manifest_v7_2026-04-12.csv}. The current aggregated control surface is \path{results/analysis/nrr_patterns_comparison_summary_r2_aggregated.csv}, with per-provider drill-down in \path{results/analysis/nrr_patterns_comparison_summary_r2_by_provider.csv} and raw comparison traces inside the three \path{results/nrr_patterns_compare_t00_r2_anthropic.zip}, \path{results/nrr_patterns_compare_t00_r2_openai.zip}, and \path{results/nrr_patterns_compare_t00_r2_gemini.zip} artifacts.

\subsection{Selected Stage B Boundary-Resolution Surface}

Selected Stage B evidence enters this manuscript as a carried-forward boundary-honesty layer, not as the paper's primary confirmatory center. In this manuscript, ``Stage B'' refers to the historical `nrr-boundary' confirmatory boundary-resolution suite over the same five delayed-commitment families, but this manuscript carries forward only the selected honesty-facing subset named here. The current NRR-Patterns source-of-truth name freeze for this layer is recorded in \path{repro/nrr_patterns_boundary_claims_manifest_v2_2026-04-12.csv}, and the paper-local rule for why these artifacts are retained here is frozen in \path{repro/nrr_patterns_stageb_selection_rule_v2_2026-04-12.md}. That rule is a paper-local integration rule rather than a manuscript preregistration freeze, and it retains \texttt{misinterpretation\_rate} plus \texttt{rework\_cost\_tokens} as the smallest downstream quality/cost pair needed to make the comparison honest. In the current NRR-Patterns package, the carried-forward Stage B read is traceable to:
\begin{itemize}
...
ablation are not part of the default mainline. Ordered composition remains
...
comparison claim hierarchy. In the current NRR-Patterns read, both therefore
...
The end of the comparison section collapses the mainline read into one compact status ladder. Table~\ref{tab:status_ladder} states the current NRR-Patterns summary.
...
Non-effective reporting is also part of the current manuscript rather than a
package-only afterthought. Table~\ref{tab:stageb_noneffective_nrr_patterns}
...
carry-forward interpretation should stop short of claiming an active effect.
...
Provider-balanced summaries are useful for the exact-replication headline, but the separated robustness member and provider-separated views are necessary where aggregation can hide reversals or tail-risk concentration. The current NRR-Patterns manuscript therefore uses four evidence roles on purpose: exact replication for the headline family-benchmark read, provider-separated stability for comparison-side reversals and concentration, robustness for controlled condition-shift stress, and boundary honesty for the carry-forward caveat layer. Collapsing those roles would either overstate the comparison or overinflate the boundary section into a second main paper.
...
This manuscript cites traceable comparison and Stage B values directly from
shipped claim-trace artifacts, and the naming freeze is recorded in the
current manifests. The released artifact set is auditable rather than a full
rerun bundle: it ships the manuscript assets, the comparison and boundary
manifests, a checksum manifest, and the named comparison/boundary artifacts
needed for inspection. In this package, exact replication means the fixed
matched-run outputs can be audited and drilled into from shipped summaries plus
preserved run-specific members; it does not mean the distributed bundle is
itself a full independent rerun environment. The exact-replication headline has
one summary artifact for inspection while preserving the upstream run-specific
members for drill-down, and the separated robustness member remains visible
rather than folded into that headline mean. Provider sensitivity remains real
and must not be hidden by a single aggregate summary, and the five-pattern map
is carried by unequal paired-row volume across patterns. That paired-row volume
is useful for auditability, but it is not the same thing as independent
support thickness. Ordered composition is intentionally demoted out of the
default mainline, and no-position ablation remains appendix-first rather than
main-text essential. These are scope controls rather than unfinished
claim-layer work.
...
The current NRR-Patterns manuscript has an auditable claim-trace home rather
than a full end-to-end rerun bundle. The package-level naming freeze is
recorded in \path{repro/nrr_patterns_package_map_v20_2026-04-12.md}, the
paper-local split comparison contract is recorded in
\path{repro/nrr_patterns_split_comparison_contract_v13_2026-04-12.md}, the
paper-local Stage B selection rule is recorded in
...
comparison claim-trace members are:
\begin{itemize}
...
Appendix-first materials include the no-position ablation
...
current default audit bundle. These appendix-first materials are therefore not
promoted into the NRR-Patterns mainline, and only the no-position ablation is
shipped in the current audit bundle.

The current NRR-Patterns manuscript checksum manifest is
...
\path{scripts/verify_audit_surface.py} verifies the shipped claim-trace files
...
comparison headline summary, the release-manifest checksum anchor,
the audit checksum manifest, and the current NRR-Patterns manuscript checksum
manifest define the current NRR-Patterns manuscript artifact set for this
manuscript.
