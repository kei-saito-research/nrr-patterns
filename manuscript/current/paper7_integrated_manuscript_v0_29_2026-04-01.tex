...
NRR should be read here as a derivation-and-evaluation framework, not as a closed recipe. The five delayed-commitment patterns in integrated paper7 are an evaluated set under the current fixed matched-run paired spine plus explicitly separated robustness and honesty layers, not an exhaustive set of admissible patterns. Additional patterns or operator-family extensions are in scope only when they are tested under the same paired-unit discipline and reported with equally explicit condition-bounded boundary language.

Integrated paper7 compares five delayed-commitment patterns on a fixed matched-run paired spine and then adds separate provider-separated stability, temperature-robustness, and selected Stage B honesty layers to show where that map weakens, reverses, or becomes non-effective.
...
The primary experimental spine uses the exact-replication five-pattern comparison surface currently carried by the released Principles line. Here, exact replication continues to mean the fixed auditable matched-run output surface named in the shipped artifacts, not a rerun-capable environment. The shipped provider/model scope is Anthropic Claude Sonnet 4 (\texttt{claude-sonnet-4-20250514}), Gemini 2.0 Flash (\texttt{gemini-2.0-flash}), and OpenAI GPT-4o mini (\texttt{gpt-4o-mini-2024-07-18}):
\begin{itemize}
...
Selected Stage B evidence is carried into integrated paper7 only to make the comparison map honest about where provider-separated quality behavior weakens, reverses, or becomes non-effective. This Stage B layer is the selected carry-forward subset of the historical `nrr-boundary' confirmatory suite, not a new paper7-native experiment block. The retained \texttt{misinterpretation\_rate} plus \texttt{rework\_cost\_tokens} pair is not presented as an original paper7 preregistration freeze; it is a paper-local integration rule that keeps the smallest downstream quality/cost pair needed for honesty reporting. Table~\ref{tab:stageb_selected_integrated} summarizes the two key metrics retained for this role.
...
manifest define the current integrated manuscript package surface for paper7.
...
A.~Chevalier et al. (2023). \textit{Adapting Language Models to Compress Contexts}. arXiv:2305.14788.
